...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
				...
				% Get the all right sensors distance info
				S = [3:5;ir_distances(3:5)']';
				% Sort by distance
				S_sorted = sortrows(S,2);
                % Prevent jump to front sensor (sensor 3)
                if (S_sorted(2,1)==3) && (abs(S_sorted(2,2)-S_sorted(2,2))<=0.00001)
                    s1 = S_sorted(1,1);
				    s2 = S_sorted(3,1);
                    % fprintf('Prevent jump\n');
                else
                    % Get the index of the 2 sensors of smaller distance
                    s1 = S_sorted(1,1);
				    s2 = S_sorted(2,1);
                end
				% smaller id to p2, bigger id to p1
                p_1 = ir_distances_wf(:,max(s1,s2));
                p_2 = ir_distances_wf(:,min(s1,s2));
                fprintf('in the fw control right\n');
            else
				...
				% smaller id to p1, bigger id to p2
                p_1 = ir_distances_wf(:,min(s1,s2));
                p_2 = ir_distances_wf(:,max(s1,s2));
                fprintf('in the fw control left\n');
            end
