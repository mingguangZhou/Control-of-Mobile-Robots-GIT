...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %% SUPERVISOR Constructor
            ...
            obj.eventsd{8} = struct('event', 'front_blocked', ...
                                    'callback', @front_blocked);
        ...
        %   See also controller/execute
            ...
            % clear_check = obj.check_event('obstacle_cleared');
			...
			elseif(obj.check_event('obstacle_cleared')) && (obj.check_event('progress_made'))
				obj.switch_to_state('go_to_goal');
                fprintf('cleared and go to goal\n');

            elseif (obj.check_event('unsafe')) && ~obj.out_of_maze
				obj.switch_to_state('avoid_obstacles');
            	fprintf('unsafe and pure OA\n');
            
            elseif (~obj.check_event('obstacle_cleared') && (obj.check_event('front_blocked')))
% 				if(obj.check_event('sliding_left'))
%                     obj.out_of_maze = false;
% 					inputs.direction = 'left';
% 					set_progress_point(obj);
%                     % fprintf('new progress point set\n');
...
%                     % fprintf('new progress point set\n');
% 					obj.switch_to_state('follow_wall');
%                 	fprintf('follow wall right\n');
% 				else
% 					obj.switch_to_state('ao_and_gtg');
%                     fprintf('blended AO GTG\n');
                   obj.switch_to_state('follow_wall');
                     fprintf('follow wall\n');
                     obj.out_of_maze = false;
%               end
            ...
            elseif (obj.check_event('unsafe')) || (~obj.is_in_state('follow_wall') && ~obj.check_event('obstacle_cleared')...
                    && obj.out_of_maze)
				obj.switch_to_state('ao_and_gtg');
            	fprintf('unsafe/with obstacle not following wall, blended\n');

            

            elseif obj.is_in_state('avoid_obstacles') && (obj.check_event('obstacle_cleared'))
                obj.switch_to_state('go_to_goal');
                fprintf('obstacle avoided and go to goal\n');
				
            elseif(obj.check_event('progress_made')) && obj.is_in_state('follow_wall') 
				% obj.switch_to_state('ao_and_gtg');
                obj.switch_to_state('go_to_goal');
                fprintf('Progress made, go to goal\n');
                obj.out_of_maze = true;

%             elseif(obj.check_event('obstacle_cleared')) && obj.is_in_state('follow_wall') 
% 				% obj.switch_to_state('ao_and_gtg');
%                 obj.switch_to_state('go_to_goal');
%                 fprintf('Space cleared, go to goal\n');
            ...
            if (~obj.is_in_state('stop'))
               set_progress_point(obj); 
               fprintf('current_dist:%.3f, %.3f, %.3f, %.3f, %.3f\n', ...
                    ir_distances(1),ir_distances(2),ir_distances(3),ir_distances(4),ir_distances(5));
               fprintf('Out of maze: %i\n', obj.out_of_maze);
            end
            ...
            if ((ir_distances(1)+ir_distances(2)) < (ir_distances(4)+ir_distances(5)))
                inputs.direction = 'left';
            else
                inputs.direction = 'right';
            end
...
%                 fprintf('now sliding left\n');
        ...
        function rc = sliding_right(obj, state, robot)
            ...
            %% START CODE BLOCK
            vector_gtg_ao=[u_gtg,u_ao];
            sigma = vector_gtg_ao \ u_fw;  % Left division
            %% END CODE BLOCK
...
%                 fprintf('now sliding right\n');
        ...
        function rc = obstacle_cleared(obj, state, robot)
            ir_distances = obj.robot.get_ir_distances();
                         % Assume initially that the robot is clear of obstacle

            % fprintf('Not cleared\n');
            % Loop through and test the sensors (only front set)
            if all(ir_distances(1:5) > obj.d_cleared)
                fprintf('cleared dist:%.3f, %.3f, %.3f, %.3f, %.3f\n', ...
                    ir_distances(1),ir_distances(2),ir_distances(3),ir_distances(4),ir_distances(5))
                rc = true;
                % fprintf('checked clear\n');
            else
                rc = false; 
                % fprintf('not clear\n');
            end
        ...
        function rc = front_blocked(obj, state, robot)
            ir_distances = obj.robot.get_ir_distances();
            % fprintf('Front sensor: %0.3f\n', ir_distances(3));
            
            % Loop through and test the sensors (only front set)
            if (ir_distances(3) < obj.d_cleared)
                rc = true;
                % fprintf('Front is blocked\n');
            else
                rc = false; 
                % fprintf('Front is not blocked\n');
            end
        end
