    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            if(obj.is_blending)
				...
				% Implement the logic for switching to stop when at goal is
                % true,
				if(obj.check_event('at_goal'))
                    obj.switch_to_state('stop');
                    % fprintf('In the range, stop');
				% switching to go to goal when obstacle cleared is true,
				elseif(obj.check_event('obstacle_cleared'))
					obj.switch_to_state('go_to_goal');
                % switching to avoid obstacles when unsafe is true,
                elseif(obj.check_event('unsafe'))
					obj.switch_to_state('avoid obstacles');
				% and switching to ao and gtg, when at obstacle is true.
